...
const ComputationsModal = ({
  ...
  <Modal ariaHideApp={false} isOpen={isOpen} onRequestClose={onCloseModal}>
    <input type="text" onChange={e => onInputChange(e)} value={query} />
    <button onClick={() => onCloseModal()}>Close</button>
    <ul>{visibleComputations.map((c, index) => <li key={index}>{c}</li>)}</ul>
  </Modal>
);
...
export default connect(
  state => ({
    ...
    visibleComputations: state.calculator.computations
      .map(c => c.toString())
      .filter(c => c.includes(state.modal.query)),
